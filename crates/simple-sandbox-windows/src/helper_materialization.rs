...
const RESOURCES_DIRNAME: &str = "simple-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "simple-command-runner.exe",
        }
    }
...
fn sibling_source_path(kind: HelperExecutable) -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("SIMPLE_SANDBOX_RUNNER_EXE") {
...
fn source_path_for_exe(exe: &Path, file_name: &str) -> Option<PathBuf> {
    ...
    if dir.file_name().is_some_and(|name| name == "deps") {
        let dev_candidate = dir.parent()?.join(file_name);
...
mod tests {
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("simple-command-runner.exe");
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let helper = resources_dir.join("simple-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved = source_path_for_exe(&exe, /*file_name*/ "simple-command-runner.exe")
            .expect("helper path");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("codex.exe");
        let sibling_helper = release_dir.join("simple-command-runner.exe");
        let resource_helper = resources_dir.join("simple-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "simple-command-runner-test-suffix.exe");
    }
}
