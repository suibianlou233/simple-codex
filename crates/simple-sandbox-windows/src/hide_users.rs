...
pub fn hide_newly_created_users(usernames: &[String], log_base: &Path) {
    ...
    hide_existing_profile_directories(usernames, log_base);
}

/// Hide profile directories that already exist from an earlier setup or command run.
/// Newly created profiles are also hidden by `hide_current_user_profile_dir` when the
/// command runner logs on as that sandbox account for the first time.
fn hide_existing_profile_directories(usernames: &[String], log_base: &Path) {
    let Some(users_root) = users_profile_root() else {
        log_note(
            "hide users: unable to resolve the Windows user profile root",
            Some(log_base),
        );
        return;
    };
    for username in usernames {
        let profile_dir = users_root.join(username);
        ...
        match hide_directory(&profile_dir) {
            Ok(true) => log_note(
                &format!(
                    "hide users: existing profile dir hidden for {username} ({})",
                    profile_dir.display()
                ),
                Some(log_base),
            ),
            ...
            Err(err) => log_note(
                &format!(
                    "hide users: failed to hide existing profile dir for {username} ({}): {err}",
                    profile_dir.display()
                ),
                Some(log_base),
            ),
...
fn users_profile_root() -> Option<PathBuf> {
    ...
    std::env::var_os("SystemDrive").map(|drive| PathBuf::from(drive).join("Users"))
}
