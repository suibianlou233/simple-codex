...
mod windows_impl {
    ...
    use crate::ipc_framed::{EmptyPayload, FramedMessage, write_frame};
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                ...
                (
                    ...
                    vec![
                        caps.workspace,
                        crate::cap::workspace_cap_sid_for_cwd(sandbox_home, cwd)?,
                    ],
                )
            }
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        (|| -> Result<CaptureResult> {
            ...
            let cancellation_thread = cancellation.map(|cancellation| {
                let capture_done = Arc::clone(&capture_done);
                let mut cancellation_pipe = pipe_write.try_clone();
                std::thread::spawn(move || {
                    while !capture_done.load(Ordering::Acquire) {
                        if cancellation.load(Ordering::Acquire) {
                            if let Ok(pipe) = cancellation_pipe.as_mut() {
                                let _ = write_frame(
                                    pipe,
                                    &FramedMessage {
                                        version: 1,
            ...
            let capture_result = (|| -> Result<(i32, bool)> {
                loop {
            ...
            let (exit_code, timed_out) = capture_result?;
