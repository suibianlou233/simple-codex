...
use serde::Serialize;
use simple_windows_sandbox::LOG_FILE_NAME;
use simple_windows_sandbox::SETUP_VERSION;
use simple_windows_sandbox::SetupErrorCode;
use simple_windows_sandbox::SetupErrorReport;
use simple_windows_sandbox::SetupFailure;
use simple_windows_sandbox::add_deny_write_ace;
use simple_windows_sandbox::canonicalize_path;
use simple_windows_sandbox::convert_string_sid_to_sid;
use simple_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use simple_windows_sandbox::ensure_allow_write_aces;
use simple_windows_sandbox::extract_setup_failure;
use simple_windows_sandbox::hide_newly_created_users;
use simple_windows_sandbox::install_wfp_filters;
use simple_windows_sandbox::is_command_cwd_root;
use simple_windows_sandbox::load_or_create_cap_sids;
use simple_windows_sandbox::log_note;
use simple_windows_sandbox::path_mask_allows;
use simple_windows_sandbox::sandbox_bin_dir;
use simple_windows_sandbox::sandbox_dir;
use simple_windows_sandbox::sandbox_secrets_dir;
use simple_windows_sandbox::string_from_sid_bytes;
use simple_windows_sandbox::to_wide;
use simple_windows_sandbox::workspace_cap_sid_for_cwd;
use simple_windows_sandbox::write_setup_error_report;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(sandbox_home) = std::env::var("SIMPLE_SANDBOX_HOME") {
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.sandbox_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.sandbox_home, &payload.command_cwd)?;
    ...
    if payload.read_roots.is_empty() {
        ...
        // Read ACLs are installed once during elevated setup. Per-command refreshes only
        // update workspace write/deny ACLs; spawning a detached reader here can keep the
        // setup executable open and needlessly rescan large system roots on every command.
        log_line(
            log,
            "setup refresh: read ACLs unchanged; skipping read ACL helper",
        )?;
    } else {
