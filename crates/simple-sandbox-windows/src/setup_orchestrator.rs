...
pub const OFFLINE_USERNAME: &str = "SimpleSandboxOffline";
pub const ONLINE_USERNAME: &str = "SimpleSandboxOnline";
...
pub fn sandbox_bin_dir(sandbox_home: &Path) -> PathBuf {
    ...
    #[cfg(not(test))]
    {
        std::env::var_os("PROGRAMDATA")
            .map(PathBuf::from)
            .map(|root| root.join("Simple").join("Sandbox").join("bin"))
            .unwrap_or_else(|| sandbox_home.join(".sandbox-bin"))
    }
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, sandbox_home);
...
fn run_setup_refresh_inner(
    ...
    log_note(
        ...
        Some(&sandbox_dir(request.sandbox_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
pub(crate) fn gather_read_roots(
    ...
    gather_legacy_full_read_roots(command_cwd, policy, sandbox_home)
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "SIMPLE_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    if let Some(path) = std::env::var_os("SIMPLE_SANDBOX_SETUP_EXE") {
    ...
    {
        let candidate = dir.join("simple-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // Packaged helpers live under `simple-resources/` next to Simple.
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("simple-resources")
            .join("simple-windows-sandbox-setup.exe");
        ...
        {
            let dev_candidate = parent.join("simple-windows-sandbox-setup.exe");
            if dev_candidate.is_file() {
                return dev_candidate;
            }
        }
    }
    PathBuf::from("simple-windows-sandbox-setup.exe")
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.sandbox_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        gather_read_roots(request.command_cwd, request.policy, request.sandbox_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, sandbox_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to SIMPLE_SANDBOX_HOME or anything under SIMPLE_SANDBOX_HOME/.sandbox,
    // SIMPLE_SANDBOX_HOME/.sandbox-bin, or SIMPLE_SANDBOX_HOME/.sandbox-secrets. These locations contain sandbox
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "SIMPLE_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    fn expanded_write_roots_still_drop_protected_sandbox_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let sandbox_home = user_profile.join("SimpleSandboxHome");
        let documents = user_profile.join("Documents");
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let sandbox_home = tmp.path().join("codex-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &sandbox_home);
        let expected =
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![extra_write_root.clone()],
            network_access: false,
            exclude_tmpdir_env_var: true,
            exclude_slash_tmp: true,
        };
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &sandbox_home);
