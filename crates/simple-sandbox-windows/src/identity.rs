...
pub fn require_logon_sandbox_creds(
    ...
    let needed_read = read_roots_override
        .map(<[PathBuf]>::to_vec)
        .unwrap_or_else(|| gather_read_roots(command_cwd, policy, sandbox_home));
    ...
    // NOTE: Do not add SIMPLE_SANDBOX_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(sandbox_home)? {
